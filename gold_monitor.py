...
# 邮箱账号密码通过 GitHub Secrets 传入，代码中不留痕迹
EMAIL_CONFIG = {
...
def get_current_gold_price():
    ...
    if not result["price_usd_oz"]:
        try:
            ...
            obj = json.loads(data)
            result["price_usd_oz"] = obj["chart"]["result"][0]["meta"]["regularMarketPrice"]
    ...
    try:
        ...
        result["usd_cny_rate"] = json.loads(data)["rates"]["CNY"]
    except Exception:
...
def get_historical_close(target_date_str):
    try:
        ...
        best_close, best_diff = None, float("inf")
...
def get_gold_news():
    ...
    try:
        ...
        print(f"新闻获取失败: {e}")
    if not news_items:
        news_items.append({"title": "（新闻数据源暂不可用）", "link": "", "source": "", "pub_date": ""})
    return news_items
...
def build_email_html(prices, prev_close, last_friday_close, news_items, drivers, upcoming_events):
    ...
    def change_color(val):
        return "#888" if val is None else ("#e74c3c" if val >= 0 else "#27ae60")
    def change_icon(val):
        return "➖" if val is None else ("📈" if val >= 0 else "📉")
...
<div class="price-section">
  ...
  <p class="sub-info">📡 {prices['source']} | 国际: ${prices['price_usd_oz']:.2f}/oz | 汇率: ¥{prices['usd_cny_rate']:.2f}</p>"""
    ...
    if GOLD_GRAMS > 0:
        html += f"""
  <div class="holdings">
    <div class="holding-item"><div class="holding-val">{GOLD_GRAMS}g</div><div class="holding-label">持仓重量</div></div>
    <div class="holding-item"><div class="holding-val">¥{current_value:,.0f}</div><div class="holding-label">当前市值</div></div>"""
        if profit is not None:
            p_color = "#e74c3c" if profit >= 0 else "#27ae60"
            p_icon = "📈" if profit >= 0 else "📉"
            html += f"""
    <div class="holding-item"><div class="holding-val" style="color:{p_color}">{p_icon} ¥{profit:,.0f}</div><div class="holding-label">浮动盈亏</div></div>"""
        html += "\n  </div>"
    ...
    if prev_close or last_friday_close:
        ...
        if prev_close:
            html += f"""
    <div class="comp-item"><div class="comp-label">📅 前日收盘</div><div class="comp-price">¥{prev_close:.2f}</div><div class="comp-change" style="color:{change_color(day_change)}">{change_icon(day_change)} {day_change:+.2f}%</div></div>"""
        if last_friday_close:
            html += f"""
    <div class="comp-item"><div class="comp-label">📅 上周五收盘</div><div class="comp-price">¥{last_friday_close:.2f}</div><div class="comp-change" style="color:{change_color(week_change)}">{change_icon(week_change)} {week_change:+.2f}%</div></div>"""
        html += "</div>"
    html += "</div>"

    html += '<div class="section"><p class="section-title">📊 涨跌主要影响因素</p><ul class="drivers">'
    for d in drivers:
        html += f"<li>{d}</li>"
    html += "</ul></div>"

    if news_items and news_items[0].get("link"):
        html += '<div class="section"><p class="section-title">📰 最新相关资讯</p>'
        for n in news_items[:5]:
            src = f"{n['source']} · {n['pub_date']}" if n.get("source") else ""
            html += f"""<div class="news-item"><div class="news-title"><a href="{n['link']}" target="_blank">{n['title']}</a></div>{"<div class='news-source'>"+src+"</div>" if src else ""}</div>"""
        html += "</div>"

    if upcoming_events:
        html += """<div class="section"><p class="section-title">🔮 未来可能影响金价的重要事件</p><table class="event-table"><tr><th>日期</th><th>事件</th><th>影响</th><th>金价走向</th><th>概率</th></tr>"""
        for evt in upcoming_events[:8]:
            prob_class = "prob-up" if "↑" in evt["probability"] else ("prob-down" if "↓" in evt["probability"] else "prob-neutral")
            impact_class = "high-impact" if "高" in evt["impact"] else ""
            html += f"""<tr><td>{evt['date']} ({evt['days_away']}天后)</td><td>{evt['event']}</td><td class="{impact_class}">{evt['impact']}</td><td style="font-size:12px">{evt['gold_direction']}</td><td><span class="probability {prob_class}">{evt['probability']}</span></td></tr>"""
            if evt.get("note"):
                html += f"""<tr><td></td><td colspan="4" style="color:#aaa;font-size:11px;padding-top:0">💡 {evt['note']}</td></tr>"""
        html += """</table><div class="summary-box"><strong>📋 综合研判：</strong>未来30天金价走势大概率受美联储政策预期主导。重点关注 <strong>非农就业、CPI通胀、FOMC利率决议</strong> 三大核心数据。<br><br>⚠️ <em>以上分析仅供参考，不构成投资建议。</em></div></div>"""

    html += f"""<div class="footer">⚡ 运行在 GitHub Actions 云端 | 每15分钟自动检测 | 完全免费<br>📧 由黄金智能监控系统自动生成 · {time_str}</div></div></body></html>"""
    return html
...
def send_email(subject, html_body, recipients):
    ...
    for to_email in recipients:
        ...
        msg["To"] = to_email
        msg["Subject"] = subject
        msg.attach(MIMEText("黄金价格监控报告，请使用支持HTML的邮件客户端查看。", "plain", "utf-8"))
...
def main():
    ...
    if day_change_abs >= DAILY_ALERT_PCT:
        ...
        alert_reason += (", " if alert_reason else "") + f"{direction} 日波动 {day_change_abs:.2f}%"

    if week_change_abs >= WEEKLY_ALERT_PCT:
        should_send = True
        alert_reason += (", " if alert_reason else "") + f"周波动 {week_change_abs:.2f}%"
    ...
    if should_send:
        ...
        html_body = build_email_html(prices, prev_close, last_friday_close, news_items, drivers, upcoming_events)
        subject = f"🥇 黄金监控 {now.strftime('%Y-%m-%d')} — ¥{prices['price_cny_gram']:.2f}/g"
        if alert_reason:
